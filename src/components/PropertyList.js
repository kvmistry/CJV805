...
const PropertyList = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5005/property/description").then(response => response.json()).then(json => {
          ...
          for (let i = 0; i < row; i++) {
            x.push(<div className='row row-sm-3 g-2'>
              {data.slice(count, count + 4).map((element) => {
                return (
                  <Property key={count++} id={element.id} img={element.img} name={element.type} description={element.description} type={element.type} />
                )
              })
              }
            </div>)
